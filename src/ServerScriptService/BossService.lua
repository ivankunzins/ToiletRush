...
function BossService:TriggerFlush(player)
 ...
 local flushPos=self.Arena and self.Arena:GetAttribute("BossFlushPosition") or Vector3.new(0,109,-60)
 if not root or (root.Position-flushPos).Magnitude>20 then return false end
 self.Flushing=true;fba("BOSS_FLUSH","🚽 БОСС СМЫВАЕТСЯ! 10 СЕКУНД НЕ БЬЁТ!");local m=self.Model
 if m and m.Parent then
  local start=m:GetPivot();local down=start*CFrame.new(0,-42,0)*CFrame.Angles(math.rad(8),0,math.rad(5));local d=Instance.new("CFrameValue");d.Value=start;d:GetPropertyChangedSignal("Value"):Connect(function()if m.Parent then m:PivotTo(d.Value)end end)
  TweenService:Create(d,TweenInfo.new(1,Enum.EasingStyle.Quad),{Value=down}):Play();task.wait(1)
  local r=self.Refs;r.handL.CFrame=CFrame.new(-17,23,0);r.handR.CFrame=CFrame.new(17,23,0);r.armL.CFrame=CFrame.new(-11,31,0)*CFrame.Angles(0,0,math.rad(68));r.armR.CFrame=CFrame.new(11,31,0)*CFrame.Angles(0,0,math.rad(-68));task.wait(Config.Boss.FlushHoldSeconds-1)
  if self.Defeated then d:Destroy();return true end
  local up=down*CFrame.new(0,42,0)*CFrame.Angles(math.rad(-8),0,math.rad(-5));TweenService:Create(d,TweenInfo.new(Config.Boss.FlushRecoverySeconds,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Value=up}):Play();task.wait(Config.Boss.FlushRecoverySeconds);d:Destroy();r.handL.CFrame=r.handLCF;r.handR.CFrame=r.handRCF;r.armL.CFrame=r.armLCF;r.armR.CFrame=r.armRCF
 end
 self.Flushing=false;if self.Active and not self.Defeated then fba("BOSS_RETURN","⚠ БОСС ВЫБРАЛСЯ! БЕГИТЕ!")end;return true
end
