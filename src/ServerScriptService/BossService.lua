...
local function piece(model,name,size,cf,color,material,shape)
    local p=Instance.new("Part");p.Name=name;p.Size=size;p.CFrame=cf;p.Anchored=true;p.CanCollide=false;p.CanTouch=false;p.CanQuery=true;p.CastShadow=true;p.Color=color;p.Material=material or Enum.Material.SmoothPlastic
...
local function buildBoss(arena)
    ...
    return model,folder,{armL=armL,armR=armR,handL=handL,handR=handR,head=head,base=base,handLCF=handL.CFrame,handRCF=handR.CFrame,armLCF=armL.CFrame,armRCF=armR.CFrame}
end
...
local function spawnStones(arena,damageFn)
    ...
    for floor,y in ipairs(floors) do
        for j=1,6 do
            index+=1
            local angle=math.rad((floor*73+j*47)%360);local radius=74;local pos=Vector3.new(math.cos(angle)*radius,y+3,math.sin(angle)*radius)
            ...
            prompt.Triggered:Connect(function(player)
                if not BossService.Active or BossService.Defeated or not rock.Parent then return end
                ...
                damageFn(player,Config.Boss.StoneDamage,"STONE");feedback(player,"BOSS_STONE","🪨 ПОПАДАНИЕ! БОСС ПОТЕРЯЛ 1 HP")
...
function BossService:Damage(amount,player,source)
    if not self.Active or self.Defeated then return false end
    amount=math.max(0,tonumber(amount) or 0);if amount<=0 then return false end
    self.Health=math.max(0,self.Health-amount)
    local model=self.Model;local head=model and model:FindFirstChild("Face")
    if head then
        local gui=head:FindFirstChild("BossLabel");local label=gui and gui:FindFirstChildOfClass("TextLabel");if label then label.Text="⚠ BOSS • "..self.Health.." HP" end
        local flash=Instance.new("Highlight");flash.FillColor=Color3.fromRGB(255,240,180);flash.FillTransparency=.35;flash.OutlineTransparency=1;flash.Parent=model;task.delay(.12,function()if flash and flash.Parent then flash:Destroy()end end)
    end
...
function BossService:Defeat()
    ...
    self.Defeated=true;self.Active=false;self.Flushing=false;self._attackToken+=1
    if self.Model and self.Model.Parent then
        local pivot=self.Model:GetPivot();local goal=pivot*CFrame.new(0,-8,0)*CFrame.Angles(0,math.rad(180),0)
        ...
        TweenService:Create(driver,TweenInfo.new(1.3,Enum.EasingStyle.Back,Enum.EasingDirection.In),{Value=goal}):Play();task.delay(1.5,function()if driver then driver:Destroy()end end)
    end
...
function BossService:TriggerFlush(player)
    ...
    if not root or (root.Position-Vector3.new(0,102,-72)).Magnitude>20 then return false end
    self.Flushing=true;feedbackAll("BOSS_FLUSH","🚽 БОСС СМЫВАЕТСЯ! 10 СЕКУНД НЕ БЬЁТ!")
    local model=self.Model
    if model and model.Parent then
        local start=model:GetPivot();local down=start*CFrame.new(0,-42,0)*CFrame.Angles(math.rad(8),0,math.rad(5))
        local driver=Instance.new("CFrameValue");driver.Value=start;driver:GetPropertyChangedSignal("Value"):Connect(function()if model and model.Parent then model:PivotTo(driver.Value)end end)
        TweenService:Create(driver,TweenInfo.new(1,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Value=down}):Play();task.wait(1)
        -- Hands catch the rim while the body hangs below it. This is the visual "almost escaped" moment.
        local r=self.Refs
        if r and r.handL and r.handR then
            r.handL.CFrame=CFrame.new(-17,23,0);r.handR.CFrame=CFrame.new(17,23,0)
            r.armL.CFrame=CFrame.new(-11,31,0)*CFrame.Angles(0,0,math.rad(68));r.armR.CFrame=CFrame.new(11,31,0)*CFrame.Angles(0,0,math.rad(-68))
        end
        task.wait(Config.Boss.FlushHoldSeconds-1)
        ...
        TweenService:Create(driver,TweenInfo.new(Config.Boss.FlushRecoverySeconds,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Value=up}):Play();task.wait(Config.Boss.FlushRecoverySeconds);driver:Destroy()
        if self.Model and self.Model.Parent and r then
            r.handL.CFrame=r.handLCF;r.handR.CFrame=r.handRCF;r.armL.CFrame=r.armLCF;r.armR.CFrame=r.armRCF
        end
    end
    self.Flushing=false
...
function BossService:Start(arena)
    ...
    self.DamageBoss=Instance.new("BindableFunction");self.DamageBoss.Name="DamageBoss";self.DamageBoss.Parent=folder;self.DamageBoss.OnInvoke=function(player,amount,source)return self:Damage(amount,player,source)end
            ...
            for _,player in ipairs(Players:GetPlayers()) do
                if player:GetAttribute("RoundActive")==true and player:GetAttribute("Eliminated")~=true then
                    ...
                    if humanoid and root and humanoid.Health>0 and (root.Position-Vector3.new(0,55,0)).Magnitude<=Config.Boss.AttackRange then
                        humanoid:TakeDamage(Config.Boss.AttackDamage);root.AssemblyLinearVelocity=Vector3.new(0,7,0);feedback(player,"BOSS_ATTACK","💥 БОСС УДАРИЛ! -"..Config.Boss.AttackDamage.." HP")
                    end
                end
            end
        end
