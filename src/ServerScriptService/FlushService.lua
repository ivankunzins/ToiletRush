...
function FlushService:Run(arena,shopService,dataService,stateRemote)
    ...
    if water then TweenService:Create(water,TweenInfo.new(1,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=Vector3.new(28,2.5,28),Transparency=0.05}):Play() end
    if drain then TweenService:Create(drain,TweenInfo.new(0.8,Enum.EasingStyle.Back,Enum.EasingDirection.In),{Size=Vector3.new(31,2,31)}):Play() end
    ...
    connection=RunService.Heartbeat:Connect(function(dt)
        ...
        for _,player in Players:GetPlayers() do
            ...
            local root=rootOf(player);local humanoid=humanoidOf(player)
            if root and humanoid and humanoid.Health>0 then
                ...
                elseif shopService:HasLifebuoy(player) or player:GetAttribute("RobuxLifebuoy")==true or player:GetAttribute("RobuxVest")==true then
                    ...
                    local delta=(center+orbit)-root.Position
    ...
    for remaining=Config.Flush.Duration,1,-1 do stateRemote:FireAllClients("FLUSH_TICK",remaining);task.wait(1) end
    ...
    for _,player in Players:GetPlayers() do
        ...
        if survived then
            dataService:AddCoins(player,Config.Economy.SurvivalReward);dataService:AddXP(player,Config.Progression.SurvivalXP)
            humanoid.AutoRotate=true
        else
            dataService:AddCoins(player,Config.Economy.ParticipationReward);dataService:AddXP(player,Config.Progression.ParticipationXP)
            if humanoid and humanoid.Health>0 then humanoid.Health=0 end
        end
    end
    stateRemote:FireAllClients("ROUND_RESULTS",survivedCount)
    if water then TweenService:Create(water,TweenInfo.new(1.2,Enum.EasingStyle.Quad),{Size=baseWater,Transparency=0.28,CFrame=CFrame.new(0,11.8,0)}):Play() end
    if drain and baseDrain then TweenService:Create(drain,TweenInfo.new(1,Enum.EasingStyle.Quad),{Size=baseDrain}):Play() end
end
